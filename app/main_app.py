...
from app.core.ranking import (
    _canon_vocab_from, _base_token,
)

# ---------------------------------------------------------------------------
# Fixed constants (no UI toggles)
# ---------------------------------------------------------------------------
RECENCY_MONTHS_FIXED = 24  # JD skills used within last 24 months
...
/* badges */
.badge { font-weight:800; font-size:.82rem; padding:.24rem .56rem; border-radius:999px; background:#f59e0b; color:#1f1300; border:0; }
...
# Experience display fallback (if extractor undercounts)
...
LABEL_JD_PRIORITY = "JD priority"; LABEL_BASE="Base score"; LABEL_MAIN="Top-skill bonus"
LABEL_REC="Recent-use bonus"; LABEL_WORK="On-the-job bonus"; LABEL_FINAL="Boosted score"; LABEL_IMPACT="Impact (0..5)"
def _render_skill_detail(selected_row: Dict[str, Any], per_rows: List[Dict[str, Any]], selected_skill: str) -> None:
    ...
    st.markdown(f"<div style='color:#9fb3c8;margin:.1rem 0 .25rem 0;'>{LABEL_IMPACT}: {contrib:.2f}</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='impact-wrap'><div class='impact-fill' style='width:{min(100, max(0, contrib/5*100)):.0f}%'></div></div>", unsafe_allow_html=True)
...
def render_candidate_cards(
    ...
    for idx in range(min(safe_count, len(ranked))):
        ...
        if all_skillmaps:
            ...
            skills_sorted = sorted(((k, float(v or 0.0)) for k,v in cmap_raw.items()), key=lambda kv: (-kv[1], kv[0]))
            if skills_sorted:
        ...
        with st.expander("Full resume text"):
            st.text_area(label="", value=full_text, height=420, label_visibility="collapsed", disabled=True, key=f"rt_{r.get('resume_id','')}")
...
if uploaded_files:
    with st.status("Bulk Ingestion", expanded=True) as status:
        ...
        status.write("Parsing files and extracting fields…")
...
if st.button("Rank / Re-Rank Candidates"):
    with st.status("Ranking Resumes", expanded=True) as stat:
        stat.write("Parsing JD and retrieving shortlist via index…")
        crew = RecruitingCrew(_collection, openai_model=os.getenv("OPENAI_MODEL","gpt-4o-mini"))
        jd_req_parsed, raw_ranked = crew.run(
            jd_text=jd_text or "",
            rows_all=[],  # retrieval-first path
            top_k=int(top_k)*3,  # buffer for post-dedup
